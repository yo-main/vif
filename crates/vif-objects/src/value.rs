    ...
    pub fn eq(&self, other: &Self) -> bool {
        match self {
            Self::Integer(i) => match other {
                ...
                Self::Global(Variable::Integer(j)) => i == j,
                Self::Global(Variable::Float(j)) => *i as f64 == *j,
            ...
            Self::Float(i) => match other {
                ...
                Self::Float(j) => i == j,
                Self::Global(Variable::Integer(j)) => *i == *j as f64,
                Self::Global(Variable::Float(j)) => i == j,
                Self::Boolean(true) => i == &1.0,
                Self::Boolean(false) => i == &0.0,
                _ => false,
            },
            Self::Global(Variable::Integer(i)) => match other {
                Self::Integer(j) => i == j,
                Self::Index(j) => i == j,
                Self::Float(j) => *i as f64 == *j,
                Self::Global(Variable::Integer(j)) => i == j,
            ...
            Self::Global(Variable::Float(i)) => match other {
                Self::Integer(j) => *i == *j as f64,
                Self::Index(j) => *i == *j as f64,
                Self::Float(j) => *i as f64 == *j,
                Self::Global(Variable::Integer(j)) => *i == *j as f64,
            ...
            Self::Boolean(i) => match other {
                ...
                Self::Global(Variable::Integer(1)) => *i == true,
                Self::Global(Variable::Float(v)) if v == &1.0 => *i == true,
                Self::Global(Variable::Integer(0)) => *i == false,
                Self::Global(Variable::Float(v)) if v == &0.0 => *i == false,
                _ => false,
            },
            Self::String(s1) => match other {
                Self::String(s2) => s1 == s2,
                Self::Global(Variable::String(s2)) => s1.as_str() == s2.as_str(),
                _ => false,
            },
            Self::Global(Variable::String(s1)) => match other {
                Self::String(s2) => s1.as_str() == s2.as_str(),
                Self::Global(Variable::String(s2)) => s1 == s2,
                _ => false,
            },
            _ => false,
        }
    ...
    pub fn neq(&self, other: &Self) -> bool {
        match self {
            ...
            Self::Boolean(i) => match other {
                ...
                Self::Float(v) if v == &0.0 => *i != false,
                Self::Global(Variable::Integer(1)) => *i != true,
                Self::Global(Variable::Float(v)) if v == &0.0 => *i != true,
                Self::Global(Variable::Integer(0)) => *i != false,
                Self::Global(Variable::Float(v)) if v == &1.0 => *i != false,
                _ => false,
            },
            Self::String(s1) => match other {
                Self::String(s2) => s1 != s2,
    ...
    pub fn lt(&self, other: &Self) -> Result<bool, ValueError> {
        match self {
            Self::Integer(i) => match other {
                ...
                Self::Global(Variable::Integer(j)) => Ok(i < j),
                Self::Global(Variable::Float(j)) => Ok((*i as f64) < *j),
            ...
            Self::Float(i) => match other {
                ...
                Self::Float(j) => Ok(i < j),
                Self::Global(Variable::Integer(j)) => Ok(*i < *j as f64),
                Self::Global(Variable::Float(j)) => Ok(i < j),
            ...
            Self::Boolean(i) => match other {
                ...
                Self::Global(Variable::Integer(j)) => Ok(&1 < j),
                Self::Global(Variable::Float(j)) => Ok(&1.0 < j),
                _ => return value_error!("Can't compare {self} and {other}"),
            },
            _ => return value_error!("Can't compare {self} and {other}"),
        }
        ...
        'b: 'a,
    {
        match self {
            Self::Integer(ref mut i) => match other {
                ...
                Self::Boolean(false) => (),
                Self::Global(Variable::Integer(j)) => *i += j,
                Self::Global(Variable::Float(j)) => *self = Value::Float(j + *i as f64),
            ...
            Self::Global(Variable::Integer(i)) => match other {
                ...
                Self::Boolean(false) => (),
                Self::Global(Variable::Integer(j)) => *self = Value::Integer(i + j),
                Self::Global(Variable::Float(j)) => *self = Value::Float(*i as f64 + j),
                _ => return value_error!("Can't add {self} and {other}"),
            },
            Self::Global(Variable::Float(i)) => match other {
                ...
                Self::Boolean(false) => (),
                Self::Global(Variable::Integer(j)) => *self = Value::Float(i + *j as f64),
                Self::Global(Variable::Float(j)) => *self = Value::Float(i + j),
                _ => return value_error!("Can't add {self} and {other}"),
            },
            Self::Global(Variable::String(i)) => match other {
                Self::Global(Variable::String(j)) => {
            ...
            Self::Boolean(true) => match other {
                ...
                Self::Global(Variable::Integer(j)) => *self = Value::Integer(1 + j),
                Self::Global(Variable::Float(j)) => *self = Value::Float(1.0 + j),
                _ => return value_error!("Can't add {self} and {other}"),
            },
            Self::Boolean(false) => match other {
                ...
                Self::Global(Variable::Integer(j)) => *self = Value::Integer(*j),
                Self::Global(Variable::Float(j)) => *self = Value::Float(*j),
                _ => return value_error!("Can't add {self} and {other}"),
            },
        ...
        'b: 'a,
    {
        match self {
            ...
            Self::Boolean(false) => match other {
                ...
                Self::Global(Variable::Integer(j)) => *self = Value::Integer(0 * *j),
                Self::Global(Variable::Float(j)) => *self = Value::Float(0.0 * *j),
                _ => return value_error!("Can't multiply {self} and {other}"),
            },
            _ => return value_error!("Can't multiply {self} and {other}"),
        };
